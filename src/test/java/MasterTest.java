...
public class MasterTest {
    ...
    public static void setupLogger() {
        try {
            FileHandler fh = new FileHandler("selenium_run.log", true);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // --- TENANT SELECTION ---
            String tenantXpath = "//p[contains(text(), 'Marco Secure Solutions Ltd.')]/parent::div//button";
            ...
            // --- NAVIGATION ---
            WebElement financeMenu = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[text()='Finance & Expense']")));
            financeMenu.click();

            WebElement paymentRequestsMenu = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[text()='Payment Requests']")));
            ...
            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(), 'Aurus Tech Pvt Ltd')]"))).click();

            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[text()='Select Category']"))).click();
            wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(), 'Vendor/Supplier Payments')]"))).click();
            ...
            // Date input via JS
            logger.info("Injecting date via JavaScript to prevent crash.");
            WebElement dateInput = driver.findElement(By.xpath("//input[@placeholder='DD-MM-YYYY']"));
            js.executeScript("arguments[0].value = '10-04-2026';", dateInput);
            js.executeScript("arguments[0].dispatchEvent(new Event('change'));", dateInput);

            // Amount & Payee
            driver.findElement(By.id("amount")).sendKeys("400");
            WebElement payee = driver.findElement(By.xpath("//label[contains(text(),'Payee Name')]/following-sibling::input"));
            ...
            try {
                ...
                org.openqa.selenium.io.FileHandler.copy(source, new File("screenshots/final_crash_report.png"));
            } catch (Exception ex) {
